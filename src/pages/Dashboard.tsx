        ...
        <Card>
          ...
          <CardContent>
            {recentSessions.length === 0 ? <div className="text-center py-8">
                ...
                {recentSessions.map(session => <SessionCard key={session.id} session={session} onViewDetails={setSelectedSession} />)}
              </div>}
          </CardContent>
        </Card>
      </div>

      <SessionDetailDialog session={selectedSession} open={!!selectedSession} onOpenChange={open => !open && setSelectedSession(null)} />
    </AppLayout>;
}